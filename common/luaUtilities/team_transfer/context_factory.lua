...
---@field action fun(senderTeamId: number, receiverTeamId: number, policyType: string): PolicyActionContext
...
function ContextFactory.create(springRepo)
  ...
  local function resourceTransfer(senderTeamId, receiverTeamId, resourceType, desiredAmount, policyResult)
    local policyType = resourceType == TransferEnums.ResourceType.METAL and
        TransferEnums.PolicyType.MetalTransfer or
